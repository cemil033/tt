class Program
{
    ...
    static void Main()
    {
        List<Debtor> debtors = new List<Debtor> {
            ...
            new Debtor("Bob L. Zambrano", DateTime.Parse("February 28, 1990"), "123456789", "BobLZambrano@armyspy.com", "2031 Limer Street Augusta, GA 30901", 6628),
            ...
            new Debtor("Carla A. Creaghasdasdasasd", DateTime.Parse("November 21, 1988"), "614-307-8974", "CarlaACreagh@dayrep.com", "3106 Bates Brothers Road Columbus, OH 43215", 11271),
        ...
        /* 3
        var list = debtors.FindAll(s => ((DateTime.Today.Year-s.BirthDay.Year) < 36)&& ((DateTime.Today.Year - s.BirthDay.Year) > 26)).ToList();
        list.ForEach(s => Console.WriteLine(DateTime.Today.Year-s.BirthDay.Year));
        */

        /* 4
        var list = debtors.FindAll(s => s.Debt < 5000).ToList();
        list.ForEach(s => Console.WriteLine(s));
        */

        /* 5
        var list = debtors.FindAll(s => ((s.FullName.Length > 18) && (s.Phone.Count(f => f == '7') >= 2))).ToList();
        list.ForEach(s => Console.WriteLine(s));
        */

        /* 7
        var list = debtors.FindAll(s=>(s.BirthDay.Month==12)|| (s.BirthDay.Month == 1)|| (s.BirthDay.Month == 2)).ToList();
        list.ForEach(s => Console.WriteLine(s));
        */

        /* 8
        var list=debtors.FindAll(s=>(s.Debt>debtors.Sum(f=>f.Debt)/debtors.Count)).OrderBy((c=>c.FullName.IndexOf(' ')+1)).OrderBy(t=>t.Debt).ToList();
        Console.WriteLine("\n");
        Console.WriteLine(debtors.Sum(f => f.Debt) / debtors.Count);
        Console.WriteLine("\n");
        list.ForEach(s => Console.WriteLine(s));
        */

        /*9 
        var list=debtors.FindAll(s=>s.Phone.Split('8').Count()<=1).ToList();
        list.ForEach(s => Console.WriteLine($"{s.FullName.Split(' ')[2]} {DateTime.Today.Year - s.BirthDay.Year} {s.Debt}"));
        */
        ...
        /*14
        var list=debtors.OrderBy(s=>s.Debt).ToList();
        for (int i = list.Count-1; i > list.Count - 6; i--)
        {
        ...
        */
        ...
        //15 
        //Console.WriteLine(debtors.Sum(s => s.Debt));

        /*16
        var list = debtors.FindAll(s => (int)s.BirthDay.Year < 1945).ToList();
        list.ForEach(s=>Console.WriteLine(s));
        */

        /* 18
        var list = debtors.FindAll(d => d.Phone.Split().Count()==10).ToList();
        list.ForEach(d => Console.WriteLine(d));
        */

        /* 19
        List<Debtor> debtors1 = debtors.FindAll(d => d.Debt < ((d.BirthDay.AddYears((int)(DateTime.Today.Year - d.BirthDay.Year)+1)-DateTime.Today).TotalDays/30*500));        
        debtors1.ForEach(d => Console.WriteLine(d));
        */

        /*20
        var list = debtors.FindAll(d => (d.FullName.Contains('s') && d.FullName.Contains('m') && d.FullName.Contains('i') && d.FullName.Contains('l') && d.FullName.Contains('e'))).ToList();
        list.ForEach(d => Console.WriteLine(d));
        */
    }
}